...
from reviews_api import get_ave_product_rating
...
# llm = ChatGroq(model="gapgpt-qwen-3.6", temperature=0)
...
def get_rating(product_id: int) -> str:
    ...
    """
    result = get_ave_product_rating(product_id)
...
agent = create_agent(
    tools=[search_products, get_rating, describe_product_image,checkout],
    model=llm,
    system_prompt=(
