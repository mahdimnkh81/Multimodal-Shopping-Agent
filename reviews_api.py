...
def get_ave_product_rating(product_id: int) -> dict:
...
def get_ave_ratings_for_products(product_ids: list[int]) -> list[dict]:
    if not product_ids:
        return []
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
